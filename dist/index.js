    ...
    return {
        ...
        config() {
            ...
            client.on("connect", _connection => {
                ...
                console.log(`✓ Performing hot reload on port ws://localhost:${params.wsport}`);
            });
            ...
            watcher.on("change", path => {
                if (path.endsWith(".twig") || path.endsWith(".json")) {
                    try {
                        execSync(`${params.sallaCli} theme sync -f "${path}" -id ${params.theme_id}  -store_id ${params.store_id} -draft_id ${params.draft_id}  -upload_url ${params.upload_url}`, { stdio: "inherit" });
